function hcShortCodeModalContents(element) {
  ...
  tb_show("Your HealCode Shortcodes", "#TB_inline?height=550&width=auto&inlineId=healcode-wp-shortcodes-content");
  hcResizeTb();
...
function hcShortCodeEditorInsertion(element) {
  ...
  var editor = jQuery(element).data('editor');
  hcGenerateShortCodeInsertEditor(shortCode, editor);
}

function hcGenerateShortCodeInsertEditor(shortCodeValue, wpEditor) {
  var fullShortCode = '[hc-hmw snippet="' + shortCodeValue + '"]';
  tinyMCE.editors[wpEditor].focus();
